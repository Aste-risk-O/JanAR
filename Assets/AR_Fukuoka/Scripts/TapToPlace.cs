    ...
    if (EventSystem.current.IsPointerOverGameObject(Input.GetTouch(0).fingerId)) {
        ...
        IEnumerator AnchoringAndSave(ARRaycastHit hit, Transform displayObjTransform, GeospatialPose pose)
        {
            ...
            if (anchor != null)
            {
                ...
                // --- НАШЕ ДОБАВЛЕНИЕ: ОБНУЛЯЕМ ПОЗИЦИЮ ---
                displayObjTransform.localPosition = Vector3.zero;
                displayObjTransform.localRotation = Quaternion.identity;
                // ------------------------------------------
                ...
                // --- НАШЕ ДОБАВЛЕНИЕ: ЗАПУСК ШОУ ---
                // Ищем скрипт LegendDirector внутри созданного объекта и запускаем его
                var director = displayObjTransform.GetComponentInChildren<LegendDirector>();
                if (director != null)
                {
                    director.StartShow();
                }
